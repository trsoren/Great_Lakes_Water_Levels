...
plotErieMonthly<-function(WL){
  erie_out = 'Erie_monthly_plot.pdf' #varies for each lake
  if (file.exists(erie_out)) {
    file.remove(erie_out)
  }
  ...
  mtext('Lake Erie master gauge vs lakewide average for each month', side=3, line=3, cex=2, outer=TRUE) #varies for each lake
...
plotErieYearly<-function(WL, predict=TRUE){
  ...
  if(predict){
    lines(1:(696/12)+1860, WL_ave[1:(696/12),2]-pred_interval[1:(696/12),1])
    x = 1:(696/12)+1860
...
plotSuperiorMonthly<-function(WL){
  out = 'Superior_monthly_plot.pdf' #varies for each lake
  if (file.exists(out)) {
    file.remove(out)
  }
...
plotSuperiorYearly<-function(WL, predict=TRUE) {
  sup_out = 'Superior_Yearly_plot.pdf'
  if (file.exists(sup_out)) {
    file.remove(sup_out)
  }
  pdf(file = sup_out, width = 7, height = 10, onefile = FALSE)
...
plotMichiganMonthly<-function(WL){
  out = 'Michigan_monthly_plot.pdf' #varies for each lake
  if (file.exists(out)) {
    file.remove(out)
  }
  pdf(file = out, width=12, height=8, onefile = FALSE)
...
plotMichiganYearly<-function(WL, predict=TRUE) {
  mich_out = 'Michigan_Yearly_plot.pdf'
  if (file.exists(mich_out)) {
    file.remove(mich_out)
  }
  pdf(file = mich_out, width = 7, height = 10, onefile = FALSE)
...
plotOntarioMonthly<-function(WL){
  out = 'Ontario_monthly_plot.pdf' #varies for each lake
  if (file.exists(out)) {
    file.remove(out)
  }
  pdf(file = out, width=12, height=8, onefile = FALSE)
...
#averages across each month
plotOntarioYearly<-function(WL, predict=TRUE) {
  ont_out = 'Ontario_Yearly_plot.pdf'
  if (file.exists(ont_out)) {
    file.remove(ont_out)
  }
  pdf(file = ont_out, width = 7, height = 10, onefile = FALSE)
